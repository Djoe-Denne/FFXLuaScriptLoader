...
bool TaskManager::load_from_tasks_config(const std::string& tasks_config_path) {
    ...
    // Create tasks from generic configurations
    for (auto&& config : *config_result) {
        // Check if it's a CopyMemoryConfig and create appropriate task
        if (auto* copy_config = dynamic_cast<config::CopyMemoryConfig*>(config.get())) {
            tasks_.push_back(task::make_task<memory::CopyMemoryTask>(*copy_config));
        }
        // Add other config types as needed in the future
    }
