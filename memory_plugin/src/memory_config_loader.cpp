...
MemoryConfigLoader::load_configs(
    ...
    LOG_DEBUG("MemoryConfigLoader: Loading configs for type {} from file: {}", static_cast<int>(type), file_path);
    
    if (type == app_hook::config::ConfigType::Memory) {
        return load_memory_configs(file_path, task_name);
    }
    
    LOG_ERROR("MemoryConfigLoader: Unsupported config type: {}", static_cast<int>(type));
...
MemoryConfigLoader::load_memory_configs(const std::string& file_path, const std::string& task_name) {
    LOG_INFO("MemoryConfigLoader: Loading memory configs from file: {} for task: {}", file_path, task_name);
    
    if (!std::filesystem::exists(file_path)) {
        LOG_ERROR("MemoryConfigLoader: Config file not found: {}", file_path);
        return std::unexpected(app_hook::config::ConfigError::file_not_found);
    }

    try {
        LOG_DEBUG("MemoryConfigLoader: Parsing TOML file: {}", file_path);
        ...
        if (auto memory_section = config.get("memory")) {
            if (memory_section->is_array()) {
                LOG_DEBUG("MemoryConfigLoader: Found memory section as array format");
                // Array format: [[memory]]
                auto memory_array = memory_section->as_array();
                LOG_INFO("MemoryConfigLoader: Processing {} memory operations from array", memory_array->size());
                
                for (auto&& op : *memory_array) {
                    auto memory_config = parse_memory_operation(op, task_name);
                    if (memory_config) {
                        LOG_DEBUG("MemoryConfigLoader: Successfully parsed memory operation");
                        configs.push_back(std::move(memory_config));
                    } else {
                        LOG_WARN("MemoryConfigLoader: Failed to parse memory operation from array");
                    }
                }
            } else if (memory_section->is_table()) {
                LOG_DEBUG("MemoryConfigLoader: Found memory section as table format");
                // Table format: [memory.item]
                auto& memory_table = *memory_section->as_table();
                LOG_INFO("MemoryConfigLoader: Processing {} memory operations from table", memory_table.size());
                
                for (auto& [key, value] : memory_table) {
                    if (value.is_table()) {
                        LOG_DEBUG("MemoryConfigLoader: Parsing memory operation with key: {}", std::string(key));
                        auto memory_config = parse_memory_operation(value, task_name, std::string(key));
                        if (memory_config) {
                            LOG_DEBUG("MemoryConfigLoader: Successfully parsed memory operation: {}", std::string(key));
                            configs.push_back(std::move(memory_config));
                        } else {
                            LOG_WARN("MemoryConfigLoader: Failed to parse memory operation: {}", std::string(key));
                        }
                    } else {
                        LOG_WARN("MemoryConfigLoader: Invalid memory operation format for key: {}", std::string(key));
                    }
                }
            }
        } else {
            LOG_DEBUG("MemoryConfigLoader: No memory section found in config file");
        }

        LOG_INFO("MemoryConfigLoader: Successfully loaded {} memory configurations", configs.size());
        return configs;
    } catch (const toml::parse_error& e) {
        LOG_ERROR("MemoryConfigLoader: TOML parse error in file {}: {}", file_path, e.what());
        return std::unexpected(app_hook::config::ConfigError::parse_error);
    } catch (const std::exception& e) {
        LOG_ERROR("MemoryConfigLoader: Exception while loading configs from {}: {}", file_path, e.what());
        return std::unexpected(app_hook::config::ConfigError::invalid_format);
    }
}
...
app_hook::config::ConfigPtr MemoryConfigLoader::parse_memory_operation(const toml::node& op, const std::string& task_name, const std::string& config_name) {
    try {
        if (!op.is_table()) {
            LOG_ERROR("MemoryConfigLoader: Memory operation is not a table");
            return nullptr;
        }
        ...
        if (name.empty()) {
            ...
            if (!name_node || !name_node->is_string()) {
                LOG_ERROR("MemoryConfigLoader: Missing or invalid name field in memory operation");
                return nullptr;
            }
            name = name_node->as_string()->get();
        }
        
        LOG_DEBUG("MemoryConfigLoader: Creating memory config for: {}", name);
        ...
        if (auto address_node = table.get("address")) {
            if (address_node->is_string()) {
                ...
                LOG_DEBUG("MemoryConfigLoader: Parsed address: 0x{:X}", address);
                memory_config->set_address(address);
            } else {
                LOG_ERROR("MemoryConfigLoader: Invalid address field type in memory operation: {}", name);
                return nullptr;
            }
        } else {
            LOG_ERROR("MemoryConfigLoader: Missing address field in memory operation: {}", name);
            return nullptr;
        }
        ...
        if (auto copy_after_node = table.get("copyAfter")) {
            if (copy_after_node->is_string()) {
                ...
                LOG_DEBUG("MemoryConfigLoader: Parsed copyAfter: 0x{:X}", copy_after);
                memory_config->set_copy_after(copy_after);
            } else {
                LOG_ERROR("MemoryConfigLoader: Invalid copyAfter field type in memory operation: {}", name);
                return nullptr;
            }
        } else {
            LOG_ERROR("MemoryConfigLoader: Missing copyAfter field in memory operation: {}", name);
            return nullptr;
        }
        ...
        if (auto original_size_node = table.get("originalSize")) {
            if (original_size_node->is_integer()) {
                ...
                LOG_DEBUG("MemoryConfigLoader: Parsed originalSize: {}", original_size);
                memory_config->set_original_size(original_size);
            } else {
                LOG_ERROR("MemoryConfigLoader: Invalid originalSize field type in memory operation: {}", name);
                return nullptr;
            }
        } else {
            LOG_ERROR("MemoryConfigLoader: Missing originalSize field in memory operation: {}", name);
            return nullptr;
        }
        ...
        if (auto new_size_node = table.get("newSize")) {
            if (new_size_node->is_integer()) {
                ...
                LOG_DEBUG("MemoryConfigLoader: Parsed newSize: {}", new_size);
                memory_config->set_new_size(new_size);
            } else {
                LOG_ERROR("MemoryConfigLoader: Invalid newSize field type in memory operation: {}", name);
                return nullptr;
            }
        } else {
            LOG_ERROR("MemoryConfigLoader: Missing newSize field in memory operation: {}", name);
            return nullptr;
        }
        ...
        if (auto description_node = table.get("description")) {
            if (description_node->is_string()) {
                ...
                LOG_DEBUG("MemoryConfigLoader: Parsed description: {}", description);
        ...
        if (auto enabled_node = table.get("enabled")) {
            if (enabled_node->is_boolean()) {
                ...
                LOG_DEBUG("MemoryConfigLoader: Parsed enabled: {}", enabled);
                memory_config->set_enabled(enabled);
            }
        }

        LOG_INFO("MemoryConfigLoader: Successfully created memory config: {}", name);
        return config;
    } catch (const std::exception& e) {
        LOG_ERROR("MemoryConfigLoader: Exception while parsing memory operation: {}", e.what());
        return nullptr;
    }
}

std::uintptr_t MemoryConfigLoader::parse_address(const std::string& value) {
    LOG_TRACE("MemoryConfigLoader: Parsing address string: {}", value);
