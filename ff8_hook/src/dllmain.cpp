...
// Global hook manager
ff8_hook::hook::HookManager g_hook_manager;

void InstallHooks() {
    // Initialize logging first
    if (!ff8_hook::util::initialize_logging("logs/ff8_hook.log", spdlog::level::debug)) {
        MessageBoxA(NULL, "Failed to initialize logging system", "Logger Error", MB_OK);
        return;
    }
    
    LOG_INFO("================================");
    LOG_INFO("FF8 Hook DLL - InstallHooks thread started");
    ...
    if (auto result = g_hook_manager.install_all(); !result) {
        ...
        MessageBoxA(NULL, "Failed to install hooks\nCheck logs/ff8_hook.log for details", "Hook Error", MB_OK);
        return;
    }
    ...
    const std::string success_msg = "Hooks installed successfully!\n" +
                                  ...
                                  "Check logs/ff8_hook.log for detailed information.";
...
void UninstallHooks() {
    ...
    LOG_INFO("FF8 Hook DLL shutting down");
    
    // Shutdown logging system
    ff8_hook::util::shutdown_logging();
}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID) {
    switch (reason) {
        case DLL_PROCESS_ATTACH: {
            ...
            ff8_hook::util::initialize_logging("logs/ff8_hook.log", spdlog::level::debug);
